...
def gera_columns(dados):
    ...
    dados['Data'] = dados['Dia'].apply(lambda _dt: str(_dt.strftime("%d-%B-%Y")))
    
    dados['Visitantes/Frequência manhã'] = np.round(100*np.array(dados['Visitantes manhã'])/np.array(dados['Frequência manhã']), 3)
    dados['Visitantes/Frequência noite'] = np.round(100*np.array(dados['Visitantes noite'])/np.array(dados['Frequência noite']), 3)
    dados['Visitantes/Frequência total'] = np.round(100*np.array(dados['Visitantes total'])/np.array(dados['Frequência total']), 3)
    ...
    dados['Novos Decididos/Frequência manhã'] = np.round(100*np.array(dados['ND manhã'])/np.array(dados['Frequência manhã']), 3)
    dados['Novos Decididos/Frequência noite'] = np.round(100*np.array(dados['ND noite'])/np.array(dados['Frequência noite']), 3)
    dados['Novos Decididos/Frequência total'] = np.round(100*np.array(dados['ND total'])/np.array(dados['Frequência total']), 3)
